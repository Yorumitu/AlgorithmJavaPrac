...
然后所有同学都移动到自己右边的那个大列的相同小列上，在最右大列的同学移动到最左大列。
换句话说，对于坐在第 i<n 行的同学，新位置在第 i+1 行，如果 i=n，那么新位置在第一行；
对于坐在第 j<m 大列的同学，新位置在第 j+1 大列，如果 j=m，那么新位置在第一大列；对于坐在第 k 小列的同学，新位置仍然在第 k 小列。
...
public class test1 {
    ...
    public static int function(int n, int m, int a){
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++){
                if (matrix[i][j]%10 == matrix[(i+1)%n][(j+1)%m]%10)
                    res++;
                if (matrix[i][j]/10 == matrix[(i+1)%n][(j+1)%m]/10)
                    res++;
            }
    ...
    public static void main(String[] args) {
        ...
        int m = sc.nextInt();
        int a = sc.nextInt();
        System.out.println(function(n, m, a));
    }
}
